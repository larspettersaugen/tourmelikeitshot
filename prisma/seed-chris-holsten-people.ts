...
function parseBirthdate(ddMmYyyy: string): Date | null {
...
function mapRoleToType(role: string): string {
  ...
  if (r.includes('tour manager') || r.includes('production manager') || r.includes('tm2') || r.includes('management')) return 'tour_manager';
  return 'crew';
}
...
async function main() {
  // Delete all people (cascade handles Invite, GroupMember; SetNull on TravelGroupMember, Contact)
